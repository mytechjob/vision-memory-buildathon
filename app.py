...
def main():
    ...
    st.set_page_config(page_title="ScreenSage", page_icon="🔍", layout="wide")
    ...
    with col1:
        if projects:
            ...
            project_options = [
                f"{p['name']} ({p['image_count']} images)" for p in projects
            ]
            ...
            selected_option = st.selectbox("Choose a project:",
                                           project_options,
                                           index=current_index,
        ...
        if projects and st.session_state.selected_project_id:
            if st.button("🗑️",
                ...
                project_to_delete = next(
                    (p for p in projects
                     if p['id'] == st.session_state.selected_project_id), None)
                if project_to_delete:
                    if data_manager.delete_project(project_to_delete['id']):
                        st.success(
                            f"Deleted project: {project_to_delete['name']}")
                        st.session_state.selected_project_id = None
                        st.session_state.current_project_name = None
                        st.rerun()
    ...
    with st.sidebar:
        st.markdown('<div class="sidebar-header">🏗️ Project Controls</div>',
                    unsafe_allow_html=True)
        ...
        if st.session_state.get('show_project_form', False):
            ...
            with col1:
                if st.button("Create",
                             type="primary",
                             disabled=not new_project_name.strip()):
                    project_id = data_manager.create_project(
                        new_project_name.strip(), new_project_desc.strip())
                    if project_id:
        ...
        if st.session_state.selected_project_id:
            st.markdown(
                '<div class="sidebar-header">📁 Upload Screenshots</div>',
                unsafe_allow_html=True)
            st.markdown(
                f'<div style="color: #B3B3B3; font-size: 0.9rem; margin-bottom: 1rem;">Uploading to: <strong style="color: #00C9A7;">{st.session_state.current_project_name}</strong></div>',
                unsafe_allow_html=True)
        ...
        st.markdown('<div class="sidebar-header">⚡ Processing Options</div>',
                    unsafe_allow_html=True)
        ...
        processing_mode = st.selectbox(
            ...
            index=0,
            help=
            "Fast Local Vision: OCR + computer vision analysis (no API required). Detailed options: OCR + advanced AI analysis (requires API keys, more expensive).",
            label_visibility="collapsed")
        ...
        if uploaded_files and st.session_state.selected_project_id and st.button(
                "🔄 Process Screenshots", type="primary"):
            process_screenshots(uploaded_files, image_processor, openai_client,
                                gemini_client, local_vision_client,
                                data_manager, processing_mode,
        ...
        if st.session_state.selected_project_id and not uploaded_files:
            load_project_data(data_manager,
                              st.session_state.selected_project_id)
    ...
    if st.session_state.selected_project_id:
        if st.session_state.processing_complete and not st.session_state.processed_data.empty:
            ...
            project_data = data_manager.get_project_images(
                st.session_state.selected_project_id)
            if not project_data.empty:
                ...
                st.info(
                    f"📂 Project '{st.session_state.current_project_name}' is ready. Upload screenshots in the sidebar to start searching."
                )
    else:
...
def load_project_data(data_manager: DataManager, project_id: int):
    ...
    try:
        ...
        if not project_data.empty:
            ...
            st.session_state.search_index = search_engine.build_index(
                project_data)
...
def process_screenshots(uploaded_files: List, image_processor: ImageProcessor,
                        ...
                        gemini_client: GeminiClient,
                        local_vision_client: LocalVisionClient,
                        data_manager: DataManager, processing_mode: str,
                        project_id: int):
    ...
    for idx, uploaded_file in enumerate(uploaded_files):
        try:
            ...
            if "Ultra Fast" in processing_mode:
                ...
                ocr_text = image_processor.extract_text_fast(image)
                visual_description = local_vision_client.analyze_screenshot(
                    image)
                ...
                ocr_text = image_processor.extract_text(
                    image)  # Current heavy preprocessing
                visual_description = openai_client.analyze_screenshot(image)
            elif "Gemini" in processing_mode:
                ocr_text = image_processor.extract_text(
                    image)  # Current heavy preprocessing
                if gemini_client.is_available():
                    visual_description = gemini_client.analyze_screenshot(
                        image)
                else:
...
def perform_search(query: str, search_mode: str, max_results: int):
    ...
    st.markdown(
        f'<div style="color: #B3B3B3; margin-bottom: 1rem;">{len(results)} matches found for "{query}"</div>',
        unsafe_allow_html=True)
    ...
    for idx, row in results.iterrows():
        ...
        try:
            processed_time = pd.to_datetime(
                row['processed_timestamp']).strftime('%m/%d/%Y')
        except:
            processed_time = "N/A"

        # Truncate content for preview
        content_preview = str(row['ocr_text'])[:50] + "..." if len(
            str(row['ocr_text'])) > 50 else str(row['ocr_text'])
        match_reason = str(row['match_reason'])[:40] + "..." if len(
            str(row['match_reason'])) > 40 else str(row['match_reason'])

        # Create table row
        with st.container():
            col1, col2, col3, col4, col5, col6 = st.columns(
                [120, 200, 150, 150, 100, 150])

            with col1:
                # Display thumbnail
                try:
                    thumbnail_bytes = base64.b64decode(
                        str(row['thumbnail_b64']))
                    thumbnail_image = Image.open(BytesIO(thumbnail_bytes))
                    st.image(thumbnail_image, width=100)
                except:
                    st.markdown(
                        '<div style="text-align: center; color: #B3B3B3; padding: 20px;">📷</div>',
                        unsafe_allow_html=True)

            with col2:
                st.markdown(
                    f'<div class="status-indicator {status_class}"></div><strong>{row["filename"]}</strong>',
                    unsafe_allow_html=True)
                dimensions = row.get('image_dimensions', 'Unknown')
                st.markdown(
                    f'<div style="color: #B3B3B3; font-size: 0.8rem;">{dimensions}</div>',
                    unsafe_allow_html=True)

            with col3:
                st.markdown(
                    f'<div style="font-size: 0.9rem;">{match_reason}</div>',
                    unsafe_allow_html=True)
                ...
                if content_preview.strip():
                    with st.expander("View Content", expanded=False):
                        ...
                        if str(row['visual_description']).strip():
                            st.text_area("Visual Description",
                                         str(row['visual_description']),
                                         height=100,
                                         disabled=True,
                                         key=f"visual_{idx}",
                                         label_visibility="collapsed")
                    ...
                    st.markdown(
                        '<div style="color: #B3B3B3; font-size: 0.8rem;">No text content</div>',
                        unsafe_allow_html=True)

            with col5:
                st.markdown(
                    f'<div class="confidence-badge {confidence_class}">{confidence:.0f}%</div>',
                    unsafe_allow_html=True)

            with col6:
                st.markdown(
                    f'<div style="font-size: 0.8rem;">{processed_time}</div>',
                    unsafe_allow_html=True)
